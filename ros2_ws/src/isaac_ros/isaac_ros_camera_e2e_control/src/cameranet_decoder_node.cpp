...
#include "isaac_ros_camera_e2e_control/cameranet_decoder_node.hpp"
...
namespace isaac_ros_camera_e2e_control {

CameraNetDecoderNode::CameraNetDecoderNode(const rclcpp::NodeOptions &options)
    : Node("cameranet_decoder_node", options) {
  // --- パラメータ宣言 ---
  output_tensor_name_ =
  ...
  nitros_sub_ = std::make_shared<MySubscriber>(
      ...
      nvidia::isaac_ros::nitros::nitros_tensor_list_nchw_rgb_f32_t::
          supported_type_name,
      std::bind(&CameraNetDecoderNode::InputCallback, this,
                std::placeholders::_1));

  RCLCPP_INFO(this->get_logger(),
              "✅ CameraNetDecoderNode initialized (tensor='%s' → "
              "topic='autonomous/cmd_drive')",
              output_tensor_name_.c_str());
}

CameraNetDecoderNode::~CameraNetDecoderNode() = default;

void CameraNetDecoderNode::InputCallback(
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    isaac_ros_camera_e2e_control::CameraNetDecoderNode)
