...
- `create_hd_map_from_vslam_bag.sh`: 保存済み cuVSLAM map を読み直した landmarks/path の snapshot を下絵 PNG/YAML にして、lane の `left_bound` / `right_bound` / `centerline` を描く editable HD map YAML、primary centerline CSV、raceline CSV まで作る実験用 flow。editor と `hd_map_eval` が同じ cuVSLAM map 座標を使うよう、既定では source bag を map 作成用と saved-map reference 用に 2 回 replay します。旧来の mapping 中 snapshot だけを使う場合は `--no-saved-map-reference` です。
