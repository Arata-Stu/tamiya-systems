...
Options:
  ...
  --prepare-vslam-map-alignment
                      after map generation, launch 2D map publishing + load_map VSLAM
                      for external manual_tf_alignment_node work
...
Interactive flow:
  ...
  6) 必要なら 2D map publish + load_map VSLAM を起動し、別paneで map/vslam_map を調整
...
run_post_vslam_map_alignment_prep() {
    ...
    if [ ! -t 0 ]; then
        echo "Warning: no interactive TTY is available. Skip post-map VSLAM alignment prep." >&2
        return 0
    fi

    if [ ! -f "${MAP_YAML_PATH}" ]; then
        echo "Warning: map YAML not found for post-map VSLAM alignment prep: ${MAP_YAML_PATH}" >&2
        return 0
    fi

    if [ ! -d "${VSLAM_MAP_DIR}" ]; then
        echo "Warning: VSLAM map directory not found for post-map alignment prep: ${VSLAM_MAP_DIR}" >&2
        return 0
    fi

    echo "[prep] Launch 2D map publisher + VSLAM(load_map) for manual map/vslam_map alignment"
    ...
    launch_args=(
        ...
        "use_perception_classifier:=false"
        "use_planning:=false"
        "use_magp_rl_trajectory:=false"
        "magp_rl_run_pure_pursuit:=false"
        "use_pure_pursuit:=false"
        "use_map_controller:=false"
        "use_sim_time:=false"
        "publish_map:=true"
        "map_server_use_sim_time:=false"
        "use_localization_manager:=false"
        "publish_localization_tf:=false"
        "use_section_localizer:=false"
        "enable_localization_and_mapping:=true"
        "publish_map_to_odom_tf:=false"
        "publish_vslam_map_identity_tf:=false"
        "use_vslam_map_alignment_node:=false"
    ...
    build_system_launch_cmd "system.launch.xml"
    ...
    if wait_for_service "/visual_slam/save_map" 60; then
        echo "  - /visual_slam/save_map is available"
    else
        echo "Warning: Visual SLAM did not become ready. Check log: ${POST_ALIGNMENT_LOG_PATH}" >&2
    fi

    if wait_for_topic "${VSLAM_LANDMARK_TOPIC}" 10; then
        echo "  - ${VSLAM_LANDMARK_TOPIC} is active"
    else
