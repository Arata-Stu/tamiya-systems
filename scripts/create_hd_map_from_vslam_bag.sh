...
start_vslam_snapshot_recorder() {
    ...
    local -a recorder_cmd=(
        ...
        --landmarks-topic "${landmarks_topic}"
        --output "${output_path}"
...
play_vslam_source_bag() {
    local player_log_path="$1"
    local diagnostics_label="$2"

    if [ "${PLAY_ALL_TOPICS}" = true ]; then
        echo "  - replay all topics"
        schedule_vslam_diagnostics 5 "${diagnostics_label}"
