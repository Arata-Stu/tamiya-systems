...
class PurePursuitTeacher:
    ...
    def _act_impl(self, poses_x, poses_y, poses_theta):
        ...
        target_s = jnp.mod(nearest_s + self.lookahead_distance, self.track_length)
        ...
        radius = jnp.where(
            ...
            1e6,
            (self.lookahead_distance**2) / (2.0 * waypoint_y),
        )
        steer = jnp.where(
            jnp.abs(waypoint_y) < eps,
            0.0,
    ...
    def act(self, poses_x, poses_y, poses_theta):
        return self._act_jit(poses_x, poses_y, poses_theta)
