...
from src.dataset import MultiScanDataset
from src.loss import ControlLoss
from src.model import TinyLidarNet
from src.transform import AddChannelDim, AddScanNoise, Compose, NormalizeScan
...
def main(cfg: DictConfig) -> None:
    ...
    train_transform = Compose(
        [
            ...
            AddScanNoise(std=cfg.dataset.noise_std),
            AddChannelDim(),
        ]
    )
    ...
    model = TinyLidarNet(
        input_dim=cfg.model.scan_points, output_dim=cfg.model.num_outputs
    ).to(device)
    ...
    for epoch in range(start_epoch, cfg.training.epochs):
        ...
        print(
            ...
            f"Train Loss: {train_loss:.4f}{val_log}"
        )
