...
MODEL_NAME="scan_image_classifier"
INPUT_TENSOR_NAME="input_tensor"
CONFIG_BASENAME="scan_image_classifier_config"
PRECISION="fp16"
MAX_BATCH_SIZE="1"
KEEP_VERSIONS="false"
PYTHON_CONVERT_SCRIPT="${SCRIPT_DIR}/5_export_onnx.py"
...
Options:
  --onnx PATH                 Use existing ONNX and skip 5_export_onnx.py
  --precision {fp16|fp32}     TensorRT compute precision (default: fp16)
...
resolve_config_source() {
  ...
  [[ -n "${config_source_path}" ]] || die "'${config_file}' not found in ${SCRIPT_DIR}/config"
